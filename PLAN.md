...
rules. This document specifies *what to build*, *how to structure it*, and
...
* **`Already going in that direction`** (for `tt…@b<n>`) now compares against the
  direction the plane will actually have *on arrival at the beacon*, not its direction
  at the time the command is typed.
* **In-progress altitude changes are made visible.** The original displayed only the
  plane's *current* altitude, never `new_altitude`, so a climb or descent — and a
...
* An **immediate** heading command supersedes and clears any pending delayed command.
  ...
  applies to heading commands *only*. Altitude and mark/unmark/ignore commands no
...
export interface Exit    extends Point { dir: Dir; }
export interface Airport extends Point { dir: Dir; }
...
way to match the original: scan `[A-Za-z]+`; if the run *starts with* a keyword, emit
...
// ...
export interface BuiltinGame { name: string; source: string; }
...
export class Game {
  ...
  constructor(def: GameDef, private rng: Rng = Math.random) {}
}
```
...
```ts
export interface GameOver { planeLetter: string | null; message: string; }
```
...
Both tables must be present as literals *and* covered by a unit test asserting the
mirror relation.
...
4. **Reap**: remove *gone* planes from `air`, incrementing `safePlanes` for each.
...
```ts
export interface PathStep { x: number; y: number; dir: Dir; }
...
* **`plane.pending` is deliberately ignored.** A plane has exactly one pending slot, so
  the delayed command now being validated will *replace* any existing one (§8.5).
...
* **Backspace** pops the last fragment, restoring `state` and `col` from that
  fragment's `state` and `col` fields (which is why `Frag.state` is the *pre*-token
...
Actions run left-to-right over a **draft** as tokens are *committed* — i.e. all actions
...
*which* kind of command was entered, not by value comparison:
...
overline`) or `descending` (`text-decoration: underline`). This is deliberately *not*
...
Step 5 is deliberately **additive** and ordered *after* the `---------` filler, which
...
Sort comparator, descending: `planes`, then `ticks`, then *ascending* `realTimeSec`
...
3. **Whether to prompt for a name** is decided with `qualifies(candidate, ANY)`,
   where `ANY` is the weakest case: a name that does *not* collide with any existing
...
* A plane with a pending command **keeps turning** (it is not frozen), resolves the
  ...
  *following* tick, and promotes `unmarked` → `marked` on resolution.
...
* `c@b1` sets a *pending* circle (the plane does not start circling immediately), and
  is accepted even though `delayAtBeacon` rejects planes that are *already* circling.
...
* A plane that is mid-turn **and** has a pending command displays the *pending*
...
* A ground plane cleared for take-off renders `Holding @ A<n> ↑<n>`, distinct from an
  uncleared one.
* The indicator is appended *after* `---------`, so an unmarked, undirected, climbing
  plane shows both.
